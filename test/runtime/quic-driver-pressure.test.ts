/**
 * QUIC driver pressure tests.
 *
 * Validates the current single-port QUIC server path under high concurrency,
 * backpressure, sustained rounds, and rapid connection churn — the same
 * pressure patterns that previously uncovered bugs in both kqueue and
 * io_uring-backed paths.
 *
...
describe('QUIC driver pressure tests', () => {
  ...
  it('sustained rounds — 8 connections × 5 rounds × 40 streams × 64KB', { timeout: scaleStressTimeout(40_000) }, async () => {
    const snap = snapResources();
    ...
    const rounds = 5;
    const streamsPerRound = 40;
    const expectedStreams = connectionCount * rounds * streamsPerRound;

    const { clients, errors: connErrors } = await openConnectionsBatched(
      serverPort,
      connectionCount,
      connectionCount,
    );
    ...
    reportLine(`${connectionCount}×${rounds}r×${streamsPerRound}s×64KB`, totalStreams, totalErrors, snap, totalBytes);
    for (let i = 0; i < roundTimes.length; i++) {
      console.log(`    round ${i + 1}: ${roundTimes[i]}ms`);
    }

    assert.strictEqual(totalErrors, 0, `expected 0 errors, got ${totalErrors}`);
    assert.strictEqual(totalStreams, expectedStreams, `expected ${expectedStreams} streams, got ${totalStreams}`);

    // Allow some jitter, but sustained rounds should still stay well below
    // multi-second stalls if the driver path is healthy.
    const baseline = roundTimes[0];
    const maxAllowedRoundMs = Math.max(baseline * 3, scaleStressTimeout(1_000));
    for (let i = 1; i < roundTimes.length; i++) {
      assert.ok(
        roundTimes[i] <= maxAllowedRoundMs,
        `round ${i + 1} (${roundTimes[i]}ms) exceeded sustained-round budget (${maxAllowedRoundMs}ms)`,
      );
    }
  });
  ...
  // Tests driver setup/teardown churn at scale.
