...
// `TxDatagram`/`RxDatagram`/`PollOutcome`/`RuntimeDriverKind` live in the
// always-compiled `crate::datagram` module (the sans-IO `ProtocolHandler`
// trait in `event_loop.rs` and the always-compiled client handlers in
// `worker.rs`/`quic_worker.rs` reference `TxDatagram` and must keep
// compiling with `os-runtime` disabled). Re-exported here so every existing
// `use crate::transport::{Driver, TxDatagram}`-style import elsewhere in
// this (os-runtime-only) module tree keeps working unchanged.
