...
impl QuicServerHandler {
    ...
    /// `conn_handle` (the `qs_stream_send` primitive) — returns released
    /// outbound-admission units, including discarded bytes on error.
    /// Buffered bytes remain owned by this handler even when it returns zero. Mirrors `H3ServerHandler::queue_stream_send` (no
