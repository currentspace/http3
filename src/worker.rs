...
impl H3ServerHandler {
    ...
    /// `conn_handle` (the `hs_stream_send` primitive) — returns released
    /// outbound-admission units, including discarded bytes on error.
    /// Buffered bytes remain owned by this handler even when it returns zero.
    ...
    /// Whether the handler owns buffered bytes or FIN for this stream.
    ...
    pub fn close_stream(
        ...
        match conn.stream_close(stream_id, u64::from(error_code)) {
            Ok(()) => {
                let released =
                    remove_pending_response(&mut self.pending_responses, &(conn_handle, stream_id))
...
impl H3ClientHandler {
    ...
    /// Whether the handler owns buffered bytes or FIN for this stream.
    #[cfg(feature = "wasm-abi")]
    pub fn has_pending_stream_write(&self, stream_id: u64) -> bool {
        self.pending_writes.contains_key(&stream_id)
    }
    ...
    pub fn flush_pending_writes_for_handle(
        &mut self,
        batch: &mut Vec<JsH3Event>,
        conn_handle: u32,
    ) {
