...
//!
//! This bounds bytes sitting in the Rust command channel. It is intentionally
//! independent of peer ACKs and QUIC delivery: the window opens when the
//! native reactor dequeues work, so JS gets backpressure without coupling
//! Writable callbacks to network round trips.
...
impl OutboundAdmission {
    ...
    pub(crate) fn release(&self, units: usize) -> bool {
        ...
        next <= self.low_water && self.pressured.swap(false, Ordering::AcqRel)
    }
...
#[cfg(test)]
mod tests {
    use super::{OutboundAdmission, outbound_payload_units};
