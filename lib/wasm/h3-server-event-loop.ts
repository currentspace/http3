...
export class WasmH3ServerEventLoop {
  ...
  async listen(port: number, host: string): Promise<DatagramServerTransportAddress> {
    ...
    try {
      ...
      let handle: number;
      try { handle = this.core.exports.hs_new(ptr, len); }
    ...
    try {
      if (this.handle !== 0) {
        ...
        await drainUntilDone(
          () => this.core.exports.hs_is_done(this.handle) !== 0,
          () => { this.core.exports.hs_on_timeout(this.handle); this.pump(); },
        );
      }
    } finally {
      this.timer.cancel();
  ...
  private rearmTimer(): void {
    if (this.handle === 0) return;
    this.timer.arm(Number(this.core.exports.hs_timeout_ms(this.handle)), () => this.onTimerFire());
  }
}
